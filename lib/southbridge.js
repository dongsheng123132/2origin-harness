...
export function southbridge(root, { whitelist = ['demo'], auditFile = 'audit.log' } = {}) {
  ...
  // A4 修正（bugscope）：审计失败不再静默吞掉——每次 logAudit 记录是否成功，
  // act 返回时带上 audit_ok，让调用方知道"可审计"这个承诺是否还成立。
  let auditOk = true;
  ...
  // 每次动作重置审计状态（避免跨动作残留）
  function resetAudit() { auditOk = true; }
  ...
  // 批准判定。支持 RFC-0001 Trust Lane 的凭据：
  //   - 传统：expect_sha256（乐观锁）/ approval:"confirm"
  //   - 标准：credentials[] 里带 { kind:'trust.credential', type:'proof_of_read', target, value }
  function checkApproval(risk, before, args) {
    if (risk === 'low') return { ok: true, approval: 'auto' };
    const { expect_sha256, approval, credentials } = args;

    // RFC-0001 §3.2：proof_of_read 凭据
    if (Array.isArray(credentials)) {
      const por = credentials.find(c => c?.kind === 'trust.credential' && c.type === 'proof_of_read');
      if (por && por.value === before.sha256) {
        return { ok: true, approval: 'proof_of_read', credential: { type: 'proof_of_read', target: por.target } };
      }
      if (por) {
        return { ok: false, approval: 'proof_of_read', reason: `proof_of_read 不匹配：当前 ${before.sha256?.slice(0,12)}…` };
      }
    }
    ...
    return { ok: false, approval: 'none', reason: `risk=${risk} 需要 proof_of_read / expect_sha256（乐观锁）或 approval:"confirm"` };
  }

  // 动作：write / append。返回 action.result（2Origin 规范）。
  // via：通道标记（RFC-0001 §3.3 合规要求 2）——记录请求从哪条通道进来。
  // audit_ok：A4 修正——审计是否真的落盘（失败不静默，让调用方知道承诺失效）。
  return function act({ verb, relpath, content, mode = 'write', expect_sha256 = null, approval = null, credentials = null, via = 'api' }) {
    const actionId = 'act:' + crypto.randomUUID().slice(0, 8);
    const target = path.resolve(root, relpath || '');
    const base = { spec: '2origin/0.1', kind: 'action.result', action_id: actionId, verb, target: relpath, via };
    resetAudit();
    const fin = (result) => ({ ...result, audit_ok: auditOk });

    if (!relpath) {
      logAudit({ ...base, status: 'denied', reason: 'relpath 为空' });
      return fin({ ...base, status: 'denied', reason: 'relpath 为空' });
    }
    ...
    if (risk === 'denied') {
      ...
      return fin({ ...base, status: 'denied', risk: 'denied', reason: riskReason });
    }
    const appr = checkApproval(risk, before, { expect_sha256, approval, credentials });
    if (!appr.ok) {
      const r = {
    ...
    try {
      ...
      return fin({ ...base, status: 'failed', risk, approval: appr.approval, reason: e.message });
    }
