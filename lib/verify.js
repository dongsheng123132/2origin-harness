...
export function verify(stateFile, root) {
  ...
  for (const f of (state.facts || [])) {
    if (f.verified && !f.source) failed.push(`verified fact 缺 source: ${(f.claim || '').slice(0, 40)}`);
    else if (f.verified) passed.push(`fact 有source: ${(f.claim || '').slice(0, 30)}`);
  }
